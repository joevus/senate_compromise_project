...
Read Data
```{r}
american_rescue <- read.csv("data/raw_tweets/american_rescue_2021.csv") %>%
  select(c(created_at, id, id_str, text, full_text, retweet_count))
american_rescue_users <- read.csv("data/raw_tweets/american_rescue_2021_users.csv") %>%
  rename(
    user_id = id,
    user_id_str = id_str,
  ) %>%
...
```{r}
american_rescue <- american_rescue %>% filter(created_at < '2022-03-06')
...
```{r}
dfm_target_words <- bipartisan_collection_corpus_a %>%
  tokens_select(c(
    "bipartisan",
    "compromise",
    "unity",
...
textstat_frequency(dfm_target_words)
```
...
dfm_target_words_partisan

textstat_frequency(dfm_target_words_partisan)
```
...
for(i in 1:nrow(all_tweets)) {
  tweet_id <- all_tweets[i, "id"]
  bipartisan_count <- target_words_all[target_words_all$doc_id == tweet_id,]$bipartisan
...
```{r}
logit_bipartisan <- glm(bipartisan_bill ~ word_present_bipartisan, data=all_tweets, family="binomial")
...
```{r}
textplot_wordcloud(
  dtm_partisan_collection_a, max_words=100,
  rotation = 0.25,
  color = rev(RColorBrewer::brewer.pal(10, "RdBu"))
)
```
